...
class FFExerciseModelService: NSObject {
    ...
    func onTimeStart() {
        if FFBaseModel.sharedInstall.mCountDownTimeState == 0 {
            ...
            mSpendTime += Int(Date().timeIntervalSince1970) - mStartTime
    ...
    func onTimeStartLong() {
        if (FFBaseModel.sharedInstall.mCountDownTimeState == 2) {
            mFinishTime = mSpendTime
            onTimeStop()
            delegate?.callbackForShowFinishDialog(sportName())
        } else if (FFBaseModel.sharedInstall.mCountDownTimeState == 1) {
            mFinishTime = mSpendTime + Int(Date().timeIntervalSince1970) - mStartTime
            onTimeStop()
            delegate?.callbackForShowFinishDialog(sportName())
        }
    }
    ...
    func onTimeStop() {
        ...
        //mAdjustModeHandler.removeMessages(MSG_SET_PROGRESS_UP);
        //mAdjustModeHandler.removeMessages(MSG_SET_PROGRESS_DOWN);
        if FFBaseModel.sharedInstall.bleConnectStatus == 3 && FFBaseModel.sharedInstall.commandReady {
            handle.writeData(mDeInit)
            //mAdjustModeHandler.sendEmptyMessageDelayed(MSG_DEINIT_EXTEND, 50);
        }
        
        FFBaseModel.sharedInstall.mCountDownTimeState = 0
        mFlagEnd2K = false
        mFlagShowPowerSeekBar = true
//        mPowerSeekBar.setProgress(10);
...
//        mTimeText.setText("20:00");
//        mSubTimeText.setText("00:00");
//        mSubCountNumText.setText("0");
//        mTotalCountNumText.setText("0");
//        mStartBtn.setImageDrawable(mDrawableStart);
    }
    ...
    private func showTime() {
        let totalSpendTime = mSpendTime + Int(Date().timeIntervalSince1970) - mStartTime
        let totalLeftTime = 1200 - totalSpendTime
...
//                        mTotalCountNumText.setText("" + mRelaxSubStepTime.length);
    ...
    @objc func handleMessage2(what: Int) {
        if (what == MSG_SET_CTRL2_EXTTEND) {
            ...
            //                    mPowerSeekBar.setProgress((curtime * mMaxPowerValue * 10) / totaltime);
            //                    powerPogressChanged(true, curtime + 50, totaltime);
            //                } else {
            //                    mPowerSeekBar.setProgress(100);
            //                }
            ...
            //                if (curtime > 0) {
            //                    mPowerSeekBar.setProgress((curtime * mMaxPowerValue * 10) / totaltime);
            //                    powerPogressChanged(false, curtime - 50, totaltime);
            //                } else {
            //                    mPowerSeekBar.setProgress(0);
            //                }
